...
if __name__ == "__main__":
    ...
    T = 4/7
    dc, dv = find_dc_dv(T)
    vector_length = find_vector_length(min_p, error_margin, error_freq, N_list)

    #sparse_matrix_list = []
    
    information = np.array([0]*int(vector_length*4/7))
    transmited_code = np.array([0]*vector_length)#np.zeros(vector_length)
    ldpc_list = []
    ...
    test_list = ldpc_list + [Hamming(information)]
    ...
    for j in range(len(p_list)):
        ...
        for i in range(len(test_list)):
            ...
            error_prob.append(num_of_errors[i]/vector_length) # falta multiplicar por T?
    ...
    plt.plot(p_list, prob_list[0], label="LDPC N=100")
    plt.scatter(p_list, prob_list[0])
    plt.plot(p_list, prob_list[1], label="LDPC N=200")
    plt.scatter(p_list, prob_list[1])
    plt.plot(p_list, prob_list[2], label="LDPC N=500")
